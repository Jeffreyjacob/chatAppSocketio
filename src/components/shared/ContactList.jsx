...
const ContactList = ({contacts,isChannel = false}) => {
    const {selectedChatData,setSelectedChatMessage,
        setSelectedChatType, setSelectedChatData } = useAppStore()
    ...
    <div className="mt-5">
      {
        contacts?.map((contact)=>(
            ...
            :"hover:bg-[#f1f1f111]"}`} onClick={()=>handleClick(contact)}>
               <div className="flex gap-5 items-center justify-start text-neutral-300">
                 {
                    !isChannel &&  <Avatar className="h-10 w-10  rounded-full overflow-hidden">
                    {
                       contact.image ? <AvatarImage src={`${API_BASE_URL}/${contact.image}`}
                          ...
                          <div className={`
                          ${selectedChatData && selectedChatData._id === contact._id ? "bg-[#ffffff22] border border-white":`${getColor(contact.color)}`}
